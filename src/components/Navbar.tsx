...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur-md dark:border-slate-800 dark:bg-slate-900/95 transition-colors">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-2.5 sm:px-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="hidden sm:block">
            <h1 className="text-sm font-bold tracking-tight text-slate-900 dark:text-slate-100 leading-none font-mono">
              realkredit.spacepope.dk
            </h1>
          </div>
        </div>

        {/* View Switcher Tabs (with proper Danish spelling "Standardlån") */}
        <nav className="flex items-center rounded-lg bg-slate-100 dark:bg-slate-800/80 p-0.5 text-xs sm:text-sm font-medium">
          <button
            onClick={() => onSelectView('refinancing')}
            className={`flex items-center gap-1 rounded-md px-2 sm:px-2.5 py-1 sm:py-1.5 transition-all ${
          ...
          >
            ...
            <span className="sm:hidden">Omlæg</span>
          </button>
          ...
          >
            ...
            <span className="sm:hidden">2-lag</span>
          </button>
        </nav>

        {/* Actions (Share, Theme, Rates status) */}
